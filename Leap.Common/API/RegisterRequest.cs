	...
	public void Validate()
	{
		...
		switch (firstFailedRequirement.Requirement.Type)
		{
			case RequirementType.MinimumLength:
				throw new ValidationException(
					$"Password must be at least {PasswordRule.Value.MinLength} characters long.");
			case RequirementType.MaximumLength:
				throw new ValidationException(
					$"Password must be at most {PasswordRule.Value.MaxLength} characters long.");
			...
			case RequirementType.RequiredChars:
				...
				if (requiredChars is SpecificCharacterClass specific)
					throw new ValidationException(
						$"Password must contain at least one of {string.Join(' ', specific.Chars)}.");
